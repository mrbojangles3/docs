...
The default topology is Spine-Leaf with 2 spines, 2 MCLAG leaves, 2ESLAG leaves and 1 non-MCLAG leaf. Optionally, you can choose to run
...
In the default topology, the following Control Node and Switch VMs are created, the control node connections are ommitted for clarity:
...
As well as the following test servers, for clarity the command and control links from the Control Node are omitted.:
